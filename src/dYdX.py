...
def run_script():
    ...
    def on_message(ws, message):
        ...
        print(str(count) + " ", " bid " + str(best_bid) + " / " + str(dicts["bids"][Decimal(best_bid)]),
              " - ask " + str(best_ask) + " / " + str(dicts["asks"][Decimal(best_ask)]))
        ...
        if count % 149 == 0:
            ...
            try:
                if (all_positions.data["positions"][0]["side"] == "LONG") and (
                        abs(int(all_positions.data["positions"][0]["size"])) != 0):
                    position_entry_clear = max(
                        float(all_positions.data["positions"][0]["entryPrice"]) * (1 + pct_spread / 100), best_ask)
                ...
                elif (all_positions.data["positions"][0]["side"] == "SHORT") and (
                        abs(int(all_positions.data["positions"][0]["size"])) != 0):
                    position_entry_clear = min(
                        float(all_positions.data["positions"][0]["entryPrice"]) * (1 - pct_spread / 100), best_bid)
                    position_size_clear = abs(int(all_positions.data["positions"][0]["size"]))
